...
The async in-process runner executes these fixtures through the built-in agent adapter. Its current evaluation scope is limited to structured expected/forbidden tool calls and exact argument constraints; natural-language semantic expectations remain explicitly unscored. A persisted results dashboard is not included. See `docs/FIRST_VERTICAL_SLICE.md` for the manual and automated product proof.
