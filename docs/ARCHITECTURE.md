...
The HTTP adapter disables redirects, enforces a bounded total deadline and response size, and validates the literal or every DNS-resolved IPv4/IPv6 address before each turn. Production requires HTTPS; localhost, non-global network ranges and cloud-metadata destinations are rejected. Connection testing reports timeout, non-2xx, invalid JSON, invalid schema/tool events and security/network failures without returning upstream response bodies or credentials.
